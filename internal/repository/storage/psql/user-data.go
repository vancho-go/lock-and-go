...
func (r *DefaultUserDataRepository) Upsert(ctx context.Context, datum []model.UserData) error {
	...
	for _, data := range datum {
		query := `
        INSERT INTO user_data (data_id, user_id, data, created_at, modified_at)
        VALUES ($1, $2, $3, $4, $5)
        ...
        `
		if _, err = tx.ExecContext(ctx, query, data.DataID, data.UserID, data.Data, data.CreatedAt, data.ModifiedAt); err != nil {
...
func (r *DefaultUserDataRepository) Read(ctx context.Context, userID string) ([]model.UserData, error) {
	query := `
    SELECT data_id, user_id, data, created_at, modified_at
    FROM user_data
    WHERE user_id = $1;
    `
